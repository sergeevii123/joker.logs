 ...
 */
public class MergeInfoWithAPIAndMapColor {
    public static void main(String[] args) {
        Properties settings = new Properties();
        settings.put(StreamsConfig.APPLICATION_ID_CONFIG, "merge-info-with-api-and-map-color");
        ...
        kStream.outerJoin(kStreamAdm,
                ...
                .map((key, value) -> {
                    switch (key){
                        case "DashboardAPI" : return new KeyValue<>(key, YELLOW + value + BLACK);
                        case "TransactionsList" : return new KeyValue<>(key, BLUE + value + BLACK);
